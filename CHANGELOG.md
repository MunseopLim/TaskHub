...
**테스트**: 신규 13 케이스 — Hex 진입점 6종, JSON 진입점 7종. Hex 는 예전 순서(HTML → 즉시 전송)로 되돌리면 3건이, JSON 은 복구 제안을 없애면 2건이 실패한다. 복구 seed 를 처음에 잘못된 필드명으로 넣었더니 "버리기" 케이스가 **헛통과**했다 — 제안 자체가 일어나지 않아 항목이 지워졌기 때문이다. 스키마를 맞추자 드러났고, 그 경위를 테스트 주석에 남겼다. 최종 1895 passing.
...
> 남은 것: 기존의 소스 정규식 검사들을 실행 기반으로 옮기는 작업. 이번에 하네스가 생겼으므로 점진적으로 가능하다.
