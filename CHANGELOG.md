...
**테스트**: 신규 2종, 최종 2389 passing. 배포되는 스크립트의 실제 함수를 가짜 DOM 위에서 돌린다(활성 시트 없음 / 행 인덱스 초과). 가드를 되돌려 두 테스트가 각각 `Cannot read properties of null` · `of undefined` 로 깨지는 것을 먼저 확인했다.
