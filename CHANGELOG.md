...
> **pwsh 를 먼저 쓰는 것의 부작용**: PowerShell 7 은 5.1 과 완전히 같지 않다 — 예를 들어 `curl` / `wget` 이 더 이상 `Invoke-WebRequest` 의 별칭이 아니다. `pwsh.exe` 가 설치된 환경에서 5.1 의 별칭에 의존하던 `shell` 액션은 동작이 바뀔 수 있다. `shell` 의 raw 계약 자체가 0.6.47 에 들어간 것이라 그 기간에 굳어진 액션은 많지 않을 것으로 보고, `&&` 가 동작하는 쪽을 택했다.

**검증의 한계**: 이 변경의 **실행 자체는 Windows 에서 확인하지 못했다**(macOS 에서 작업). 그래서 셸 선택·연산자 판정·세 모드가 공유하는 분기 순서·one-shot 스크립트 조립을 **순수 함수로 뽑아** 플랫폼과 무관하게 단위 테스트로 고정했다(신규 6 케이스). 실제 프로세스 기동은 Windows 러너에서의 확인이 필요하다.

**테스트**: 위 6 케이스 포함 총 신규 15 케이스. 최종 1920 passing.
