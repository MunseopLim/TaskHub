...
### 수정 — 0.7.16 리뷰 반영: ScriptBlock 주입과 죽은 `output` 필드

`output` 의 각 필드가 **언제 살아 있는가**를 런타임·의존성 추론·Doctor·Preview 네 곳이 다르게 알고
있었다. 이번 릴리스는 그 조건을 런타임 기준 하나로 모은다.

#### High (데이터 손실 / 신뢰 손상 / 호환성 깨짐)

- **PowerShell ScriptBlock 주입이 Doctor 를 통과했다**: 안전 문자 판정이 `{`·`}` 를 메타문자로 세지
  않아 고정 목록 `quickPick: ["{whoami}"]` 가 `pwsh -Command "Invoke-Command -ScriptBlock ${v}"` 로
  흘러가도 **경고가 0건**이었다. 중괄호를 메타문자에 넣고(PowerShell 은 스크립트 블록, POSIX 는
  brace expansion), 스크립트 자리를 **자리 단위로** 추적한다 ([src/doctor.ts](src/doctor.ts)) —
  `-ScriptBlock`·`-Action`·`-FilterScript` 류 매개변수의 값과, 스크립트 블록을 위치 인자로도 받는
  cmdlet(`Invoke-Command`·`Start-Job`·`ForEach-Object`·`Where-Object` 등)의 위치 인자가 코드 자리다.
  매개변수 이름은 접두사로 맞으므로 `-Scr` 같은 축약도 인정한다. cmdlet 을 통째로 재해석 명령으로
  두지는 않는다 — 그러면 `Invoke-Command -ComputerName ${host}` 의 컴퓨터 이름까지 경고가 붙는다.
- **쓰이지 않는 `output` 필드가 가짜 의존성과 순환을 만들었다**: 제외 목록이 정적이라 공용 순회가
  런타임이 읽지도 않는 자리를 계속 읽었다. **실제 실행상 DAG 인데 `A → B → A` 순환으로 액션 전체가
  거부**되고, 조건으로 꺼진 `A` 때문에 `B` 까지 조용히 skip 됐다. 이제 제외 경로를 태스크 상태로
  계산해 런타임 조건을 그대로 옮긴다 ([src/pipelineUtils.ts](src/pipelineUtils.ts)):
  `passTheResultToNextTask` 가 없으면 `output` subtree 전체가 죽고, 그 안에서도 `filePath`·
  `overwrite` 는 `mode: "file"` 일 때만 산다. `language` 는 아예 보간되지 않는다(런타임이 언어 id 로
  그대로 쓴다).
- **같은 조건을 Doctor 와 Preview 가 쓰지 않았다**: 그래프에만 적용해 두 진단이 정면으로 어긋났다 —
  꺼진 output 의 `${ghost.output}` 에 `variable.unresolved` 가, 꺼진 경로에 `path.outside-workspace`
  **에러**가 붙어 실행되지도 않는 자리를 고치라고 요구했고, Preview 는 "fix before running" 과
  "OUTSIDE WORKSPACE" 를 띄웠다. 반대로 **살아 있는** `output.overwrite` 의 참조는 아무도 보지 않아
  조용히 리터럴로 나갔다. 세 소비자가 같은 조건을 쓰고, `overwrite` 를 검사 대상에 넣었다
  ([src/doctor.ts](src/doctor.ts), [src/previewRun.ts](src/previewRun.ts)).

#### Medium (상태 무결성 / 명령 오동작)

- **실행 파일 자리의 참조를 "풀리는지" 보지 않고 펼쳤다**: `${which.typo}` 처럼 런타임에서 리터럴로
  남아 **실행조차 되지 않는** 참조를 Doctor 가 `sh` 로 펼쳐, 옳은 `variable.unresolved` 위에 없는
  주입(`command.nested-interpreter`)을 얹었다. 이제 해석 가능성을 `simulatedResultKeys` 한곳에서
  판정한다 — `variable.unresolved` 를 내는 모델과 같은 출처라 둘이 어긋날 수 없고, 태스크 타입별 키를
  따로 적을 일도 없다. 풀릴 대안이 하나도 없으면 인터프리터 진단을 아예 얹지 않는다.
- **`??` 체인의 대안 판정**: 예약 내장(`${workspaceFolder}`)과 태스크를 가리키는 bare 참조(`${pick}`)는
  **항상 풀려 체인을 끝내므로** 뒤 대안을 펼치지 않는다. 자기 자신(`${run}`)은 예외다 — 도는 시점에
  자기 결과는 아직 문맥에 없어 다음 대안으로 넘어간다. 반대로 **태스크 대안은 체인을 끊지 않는다**:
  결과가 사라지는 길이 `when` 외에도 `continueOnError` 실패·취소와 전이적 skip 까지 있어, 하나라도
  빠뜨리면 뒤 대안이 실제로 실행되는데 죽었다고 단정하게 된다(과탐을 감수한다).
- **`output.ignored` 가 shell/command 로만 좁혀져 있었다**: 게이트는 타입을 가리지 않는데 진단만
  좁아서, 죽은 필드의 참조·경로 진단을 뺀 뒤로 `fileDialog` 같은 타입은 **아무 진단도 남지 않았다**.
  타입 제한을 없애고 죽은 필드를 같은 모델로 나눴다 — `mode`·`content` 는 캡처 모드에서,
  `filePath`·`overwrite` 는 거기에 더해 `mode: "file"` 에서, `language` 는 `mode: "editor"` 에서만
  살고, `capture`·`diagnostics` 는 **문자열 출력이 있을 때만** 산다(`stringManipulation` 이 플래그
  없이도 capture 를 쓰는 이유).

#### UX / 일관성

- **Preview 가 왜 안 쓰이는지 말한다**: 무시되는 블록에 사유를 함께 적는다
  (`not used — only used with mode: 'file'` 등). boolean `overwrite` 와 `language` 도 예외가 아니다.
- **capture 안내가 한 리포트 안에서 어긋났다**: 머리말은 "capture / diagnostics 는 영향 없음" 이라
  해 놓고 몇 줄 뒤에서 "captures will be skipped" 라고 말했다. 조건을 **문자열 결과가 나는가**로
  통일하고 `diagnostics` 도 함께 알린다. 경고가 `(shell|command)` 로 좁혀져 `fileDialog` 처럼 애초에
  문자열을 내지 않는 타입의 capture 가 조용히 지나가던 것도 닫혔다.

**테스트**: 신규 16 케이스, 최종 2823 passing.

## [0.7.16] - 2026-08-10

### 수정 — `??` 가 조용히 리터럴이 되던 것과, Doctor 주입 검사의 재작성

리뷰가 여러 차례 있었지만 주제는 셋이다. (1) `command` 타입에서 `??` 참조가 리터럴로 나갔다.
(2) Doctor 의 중첩 인터프리터 검사가 **값의 문자 집합**만 보던 것을 **값이 놓인 문법 자리**를 보도록
다시 썼다. (3) 런타임이 보간하지 않는 필드를 의존성·진단이 읽어 가짜 순서·가짜 경고를 만들었다.

#### High (데이터 손실 / 신뢰 손상 / 호환성 깨짐)

- **`command` 타입 + `??` 참조가 해석되지 않았다**: 명령 문자열을 공백으로 토큰화한 뒤 토큰마다
  보간하는데(보간값이 argv 경계를 넘지 못하게 하는 0.6.50 의 장치), 토크나이저가 `${…}` 를 통째로
  보지 않아 `${a.x ?? b.y}` 가 세 조각으로 부서져 **리터럴 그대로** 실행 파일에 넘어갔다
  ([src/pipelineUtils.ts](src/pipelineUtils.ts)). 같은 참조가 `shell` 타입과 `args` 배열에서는 멀쩡히
  동작해서 원인을 찾기 어려운 종류의 실패였다. 이제 `${…}` 를 한 토큰으로 보고, 끝을 **첫 `}`** 로
  잡아 보간기가 보는 경계와 맞췄다.

- **주입 검사가 값의 문자 집합만 봤다** (가장 큰 구멍): `sh -c "echo ok; ${ask.value}"` 는
  `^[A-Za-z0-9_-]+$` 로 제약된 값이라 무경고였지만 `whoami` 를 넣으면 그대로 실행된다. 메타문자가
  하나도 없어도 **자리가 명령이면 명령**이다. 후보를 토큰이 아니라 **스크립트 텍스트**로 모아 참조의
  문법 자리를 따지도록 재작성했다 ([src/doctor.ts](src/doctor.ts)). 면제는 세 조건을 모두 만족할
  때만이다 — ① 값의 모양이 제약됨, ② 값이 **데이터 자리**임, ③ 값이 **옵션이 될 수 없음**. 전체
  규칙은 [docs/features.md](docs/features.md) §`command.nested-interpreter` 에 있다.

  자리 판정은 dialect(POSIX·cmd·PowerShell)별로 인용·이스케이프·행 잇기를 지켜 가며 스크립트를
  왼쪽에서 오른쪽으로 한 번만 훑는다. 면제하지 않는 자리: 스크립트/명령 시작, `;`·`&&`·`|`·개행·
  `(`·백틱·`$(` 뒤, **리다이렉션 대상**(붙어 있든 떨어져 있든 낱말 중간에서 시작하든), `then`/`do`
  같은 예약어 뒤, `eval`·`env`·`sudo`·`trap`·`time`·`coproc`·대입 빌트인처럼 인자를 다시 코드로 읽는
  명령의 인자, `find -exec`·`printf -v` 처럼 특정 옵션 뒤, cmd 의 `%…%`·`!…!` 확장 안쪽.
  **명령 이름은 인용·이스케이프를 걷어 낸 뒤** 비교하므로 `e\val`·`ev''al` 도 `eval` 로 알아보고,
  이름이 실행 시점에 정해지면(`$CMD`·`%TOOL%`·brace/glob·명령 치환) 증명할 수 없으므로 면제하지 않는다.

  **동적으로 정해지는 인터프리터**도 새 진단 `command.dynamic-interpreter` 로 알린다 — 실행 파일이나
  스크립트 스위치가 `${…}` 면 무엇이 실행될지 알 수 없으므로 fail-closed 다. 고정 `quickPick` 처럼
  값 집합이 정적이면 열거해 실제 argv 로 판정한다.

  **변수 대입은 더 이상 안전한 자리가 아니다**(동작 변경): 이 검사는 값이 놓인 자리만 보지 그 뒤로
  어떻게 흐르는지는 보지 않으므로 `sh -c "CMD=${v}; $CMD"` 를 증명해 낼 수 없다. `TAG=${v}` 도 이제
  경고한다. 문서가 권하던 패턴도 `^[A-Za-z0-9_][A-Za-z0-9_-]*$` 로 바뀌었다 — 선행 `-` 를 허용하면
  인자가 옵션이 되어(`find … -exec`) 명령 실행으로 이어진다.

- **보간하지 않는 필드가 의존성·skip 으로 해석됐다**: 의존성 추론과 조건-skip 전파가 태스크의 거의
  모든 문자열을 훑어, 런타임이 보간하지 않는 `confirmLabel`·`validateMessage`·다이얼로그 `options`·
  `output.title` 안의 `${A.value}` 하나가 `B → A` 의존성을 만들었다. 대가는 순서가 밀리는 정도가
  아니다 — `A` 가 조건으로 꺼지면 `B` 까지 조용히 꺼지고, 반대 방향의 진짜 참조가 있으면 **가짜
  순환으로 액션 전체가 거부**된다. 제외 목록을 실제 보간 여부 기준으로 다시 세우고
  (`NON_INTERPOLATED_TASK_KEYS`), **키 이름이 아니라 경로**로 적용한다 — `env` 는 임의 키를 허용하므로
  이름만 보면 `env: { title: '${password.value}' }` 의 값이 통째로 빠져 비밀 taint 까지 놓친다.
  순회도 한 함수(`walkInterpolatedTaskStrings`)로 합쳐 의존성·skip·Preview·Doctor 가 같은 자리를 본다.

- **OS 분기를 고르기 전에 모든 분기를 보간했다**: macOS 에서 도는 액션의 windows 분기에 있는
  `${pick.value}` 가 길이 상한을 넘으면 **쓰이지도 않는 분기** 때문에 실행이 실패했다. `command` ·
  `itemsFromCommand` · Preview 를 `interpolateToolValue` 와 같은 "고르기 → 보간" 순서로 맞췄다.

- **비밀 마스킹 시 기록되는 argv 가 실제 실행과 달랐다**: `command` 타입인데 표시용 재조립이 통짜
  보간을 써서, 실제로는 인자 하나로 간 `"/My Docs/a.txt"` 가 History 에는 둘로 남았다. 비밀은 가려도
  이력이 실행을 잘못 설명한다 — 표시도 실행과 같은 토큰 경계를 쓴다.

#### Medium (상태 무결성 / 명령 오동작)

- **안전한 회피책을 주입으로 오진했다**: POSIX `sh -c` 는 **다음 인자 하나만** 스크립트이고 그 뒤는
  `$0`·`$1` 인데 뒤를 전부 이어 붙여, 문서가 권하는 형태(`sh -c '고정 스크립트' _ "${ask.value}"`)에
  경고가 붙었다 — **올바로 고쳐도 경고가 사라지지 않았다.** 인터프리터마다 삼키는 범위를 나눴다.
  셸 옵션도 정규식 흉내가 아니라 실제 모델로 걷는다: POSIX 셸은 옵션을 다 읽은 뒤 **첫 피연산자**를
  실행하므로 `sh -cx …`·`bash -cx -O extglob …` 같은 형태가 모두 잡히고, `sh /dev/null -c "…"` 처럼
  스크립트가 아닌 자리는 조용하다.
- **Preview 가 비활성 OS 분기와 리터럴 필드를 오류로 보고했다**: 현재 플랫폼이 전달되지 않아 다른 OS
  분기의 오타나 보간되지 않는 필드까지 "fix before running" 으로 올렸다. Doctor 는 설정 파일 전체를
  보는 것이 목적이므로 모든 분기를 계속 검사하고, **Preview 만** 현재 플랫폼으로 좁힌다.
- **죽은 필드를 보간했다**: `itemsFromCommand` 가 있으면 정적 `items` 는 실행되지 않고, `output.mode`
  가 `file` 이 아니면 `filePath`·`overwrite` 는 쓰이지 않는다. 결과가 버려질 뿐이지만 값이 NUL·길이
  상한에 걸리면 **쓰이지도 않는 필드 때문에** 태스크가 실패한다.
- **`when` 은 `var` 만 보간한다**: `equals`·`notEquals`·`matches`·`in` 은 비교 대상 리터럴인데 그 안의
  `${A.value}` 가 의존성으로 잡혔다. `when.literal-operand` 진단 문구도 동작에 맞췄다.
- **한 소스의 예외가 진단 전체를 지웠다**: `runDoctor(inputs)` 를 통째로 감싸고 있어 한 파일이
  실패하면 다른 파일의 결과도 게시되지 않고 기존 진단이 stale 로 남았다. 이제 소스마다 따로 돌리고
  실패한 소스에 `doctor.analysis-failed` 를 남긴다.
- **순차 파이프라인의 배리어를 줄였다**: 비-parallel 태스크마다 앞의 **모든** id 를 배리어로 복사하던
  것을 **직전 비-parallel 태스크부터**만 담는다 — 그 태스크가 이미 그 앞을 전이적으로 기다리므로 순서
  관계는 같다(무작위 그래프 10만 개에서 도달 관계 일치 확인).

#### 성능 / 견고성

큰 설정에서 진단이 초 단위로 늘어지거나 아예 죽던 자리들을 함께 고쳤다 — 모두 O(n²) 였다.

- 참조마다 스크립트를 처음부터 다시 훑던 자리 채용 참조 12,000개(약 150KB)에 6.2초 → 125ms.
- 태스크마다 `allResults` 를 복사하던 보간 문맥·전방 참조 집합, 그리고 위 배리어 복사: 태스크
  8,000개에 11.1초 → 163ms.
- 자리마다 `argv.slice(…)` 를 펼쳐 담던 후보 수집: 429KB 명령줄에 9.1초 → 3MB 가 306ms
  (1.3MB 에서는 `push(...큰 배열)` 이 인자 상한을 넘겨 `RangeError` 로 죽었다).
- 닫히지 않은 `${` 가 많을 때 `indexOf` 가 끝까지 다시 훑던 토크나이저: 500KB 에 1.1초 → 19ms.
- 중첩 치환이 깊을 때 닫는 구분자마다 스택 전체를 훑던 스캐너: 360KB 에 9.7초 → 24ms.
- `detectGraphCycle` 을 **반복형**으로 바꿨다. 위 배리어 축약으로 순차 파이프라인이 사슬이 되어 DFS
  깊이가 태스크 수만큼 자라, 12,000개에서 재귀가 `RangeError` 로 죽었다. 방문 순서와 반환 경로는
  그대로다(무작위 그래프 10만 개 대조, 불일치 0건). 긴 순환 경로는 진단 메시지에서 가운데를 접는다
  (12,000개 사슬의 메시지가 109,000자였다) — Doctor·실행·Preview 가 같은 함수를 쓴다.

**대조 검증**(각 하네스가 덮는 범위):

- **문자 집합**: 생성한 20만 개 패턴을 실제 `RegExp` 의미와 대조 — 잘못된 "안전" 판정 0건.
- **첫 글자(`-`)**: 3,000개 패턴(빈 그룹·lazy 수량자 포함)을 전수 대조 — 구멍 0건.
- **셸 옵션 모델**: 실제 `/bin/sh`·`bash` 에 900개 argv — 실행된 312건 중 놓친 것 0건.
- **문법 자리**: 46개 형태(선행 대입·리다이렉션·인용·개행·옵션 주입)를 실제 셸에 돌려 대조 — 값이
  실행되는데 면제한 경우 0건.
...
"실제로 실행될 문자열을 만드는 규칙"이 실행·Doctor·Preview·DAG·기록에 복제돼 있다는 구조는 그대로다.
필드 열거만 `NON_INTERPOLATED_TASK_KEYS` 한곳으로 모았고, 순서(타입 → OS 분기 → source/mode → 필드별
처리)를 담는 공용 함수(`collectEffectiveTaskFields`)는 아직 없다.

**조건부 죽은 필드**는 그 함수가 있어야 안전하게 닫힌다. 지금은 정적 경로만 제외하므로 다음이 아직
가짜 의존성을 만든다: `passTheResultToNextTask` 없는 태스크의 `output.*`(0.7.17 에서 닫았다), 정적
quickPick 의 `cwd`, 내장 zip/unzip 의 `env`. **빠뜨린 의존성은 가짜 의존성보다 나쁘다** — 값이 오기
전에 태스크가 실행되고, 조용하다.
