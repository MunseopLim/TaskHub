...
**테스트**: 신규 6 케이스. 예전 순서(HTML → 즉시 전송)로 되돌리면 3건이 실패한다. 최종 1888 passing.

> JSON Editor 도 같은 공백이 남아 있다 — `openJsonEditorFile` 을 실행하는 테스트가 없고, 복구 경로는 함수의 **소스 텍스트를 정규식으로** 검사한다. 다음 작업으로 둔다.
