...
> **트레이드오프**: 위 마지막 항목 때문에 비밀번호를 쓰는 태스크가 실패하면 사용자가 **원인을 볼 수 없다**(verbose 로그에도 "억제됨" 사실만 남는다). 보안 쪽에 무게를 둔 선택이며, 디버깅 난이도와 맞바꾼 것임을 여기 적어 둔다.
