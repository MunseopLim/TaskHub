...
*   **설정 파일 편집**: 각 뷰(메인, 링크, 즐겨찾기)의 제목 표시줄에 있는 연필 아이콘을 클릭하여 `.vscode` 폴더에 있는 `actions.json`, `links.json`, `favorites.json` 파일을 쉽게 열고 편집할 수 있습니다. 파일이 없으면 새로 생성됩니다.
...
- Code/RO/RW/ZI 분류별 세부 크기 ("Details ▶" 토글로 표시/숨김, region 단위 독립 동작)
...
- 테이블 메인 컬럼은 **Object** (오브젝트 파일명: `main.o`, `lludiv5.o` 등)
- "Function ▶" 버튼 클릭으로 Function 컬럼 표시/숨김 전환

### 검색 및 탐색

- **키워드 검색**: 상단 검색창에서 섹션 이름, 주소, 타입으로 필터링. 접힌 region 내부도 검색되며 매치 시 자동으로 펼침
- **Region 이동** (`Ctrl+Shift+O`): Memory Map 패널이 활성화된 상태에서 VS Code QuickPick으로 region 목록을 표시하고, 선택 시 해당 region으로 스크롤 및 펼침
