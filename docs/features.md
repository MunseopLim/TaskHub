...
- 낱말 중간(`${ask.va|lue}`)에서는 삽입/대체 두 범위를 제공하므로 `editor.suggest.insertMode` 설정이 그대로 적용됩니다 — `replace`면 꼬리(`lue`)가 사라지고 `insert`면 남습니다. 대체 범위는 그 **대안의 끝**까지이므로 `??` 뒤는 건드리지 않습니다.
