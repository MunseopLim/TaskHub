...
Scatter는 `NAME 시작주소 크기 { ... }` 헤더를 지원하며 줄바꿈을 허용합니다. 시작주소·크기는 상수 숫자이고, `+offset` 시작주소는 상위 load 영역을 기준으로 계산합니다. Scatter의 산술식·심볼·추가 속성이나 닫히지 않은 블록은 미지원·불완전 입력으로 알립니다.

선택한 링커/스캐터 파일에서 일부 영역을 해석하지 못하면 경고하고 불완전한 영역 목록을 사용하지 않습니다. 최초 열기는 저장된 영역 또는 ELF `PT_LOAD`로 폴백하고, **Refresh** 실패는 기존 결과를 유지합니다. 선언을 지원되는 상수식으로 바꾸거나 `memoryMap.regions`에 전체 영역을 직접 정의하세요.
