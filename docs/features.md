...
- 액션 실행: [기능 런처](#status-bar-기능-런처) · [액션 만들기](#8-액션-생성-마법사) · [액션 검색](#quick-action-palette-taskhub-액션-실행) · [실행 기록과 입력 프로필](#14-액션-실행-히스토리)
- 파일 확인·편집: [JSON Editor](#json-editor-커맨드) · [Hex/Text 변환기](#hextext-변환기) · [Hex Viewer](#20-hex-viewer) · [Memory Map](#19-memory-map-시각화)
- 코드 분석·설정: [C/C++ Hover](#15-cc-hover-기능) · [액션 작성 가이드](actions.md) · [Doctor](#23-taskhub-doctor-action-lint) · [설정 레퍼런스](#21-설정-레퍼런스)
...
- 목록은 TaskHub 사이드바·설정·업데이트 확인, 액션 도구, 임베디드 도구, 미리 보기로 나뉩니다. 항목의 제목과
...
**빈 표에서 시작하기:** `[]`나 `[{}]`, 마지막 행을 삭제한 표에서는 필드 이름 입력란이 나타납니다. 이름을 입력하고 **필드 추가**(또는 Enter)를 누르면 객체 행에 빈 문자열 필드를 추가하며, 객체 행이 없으면 첫 행도 만듭니다. 기존 표에서도 **필드 추가**로 열을 늘릴 수 있습니다. 이름 양끝 공백은 제거하며, 빈 이름과 기존 필드 이름은 오류로 안내합니다. Escape나 **취소**는 데이터를 바꾸지 않고 폼을 닫습니다. 행에 필드가 없을 때 **행 추가**는 이 입력란으로 포커스를 옮깁니다. 필드 생성도 Undo/Redo·복구·저장 보호를 따릅니다.

**원문 열기**는 표를 저장하거나 닫지 않습니다. 원문 옆에 미저장 표 편집을 유지하며, 객체 셀의 아직 올바른 JSON이 아닌 입력도 버리지 않습니다. 원문 파일을 변경하면 기존 외부 변경 감지 규칙이 적용됩니다.
...
- `Alt+↑` / `Alt+↓`: 행 순서 변경. 행 왼쪽의 `⠿` 그립에 포커스를 두고 누릅니다 (Tab으로 이동 가능). 이동 후 포커스는 옮겨진 행을 따라가므로 연속으로 누를 수 있고, 이동 결과는 스크린리더에 알림으로 전달됩니다. 마우스 드래그 방식도 그대로 동작합니다.

#### 지역화 / 접근성 (0.6.19부터)

- 웹뷰 안의 모든 문자열이 VS Code 언어 설정을 따릅니다 (한국어 / 영어).
- 아이콘만 있는 버튼(↶ ↷ ✕ ⠿ a→s s→a s→# #→s)에 `aria-label`이 붙습니다.
- **버튼 조작 영역**: 삭제 버튼과 값 변환 배지는 최소 24×24 크기의 클릭·터치 영역을 제공합니다.
...
- **셀 편집 진입**: 셀에 Tab으로 이동해 `Enter` 또는 `Space`로 편집을 시작할 수 있습니다.
- **시트 탭**: `Tab`으로 탭 줄에 진입하고 `←` / `→`로 이동합니다(양끝에서 순환). 스크린리더에도 현재 시트가 전달됩니다.

웹뷰 구성은 [아키텍처](architecture.md#프로젝트-구조), 버전별 개선 이력은 [CHANGELOG](../CHANGELOG.md)를 참고하세요.
...
*   **행 액션**: 마우스를 올리면 링크 행 오른쪽에 **통합 브라우저에서 열기**와 **URL 복사** 아이콘이 표시됩니다. 브라우저 아이콘은 올바른 HTTP(S) 링크에만 표시되며, 클릭하면 VS Code 안에서 해당 주소를 엽니다. Integrated Browser가 없는 구형 VS Code에서는 Simple Browser로 엽니다. 키보드 사용자는 `Shift+F10`의 우클릭 메뉴에서 링크 열기·통합 브라우저에서 열기·URL 복사·편집·삭제에 접근할 수 있습니다.
*   **링크 추가 / 편집**: **+**를 누르면 클립보드에 있는 올바른 URL을 입력란에 미리 채웁니다. URL을 확인한 뒤 HTTP(S) 페이지의 제목을 가져와 링크 이름으로 제안하며, 실제 링크에는 입력한 URL을 저장합니다. 조회 중에도 제목을 수정하거나 바로 확정할 수 있고, 직접 고친 제목은 덮어쓰지 않습니다. 제목을 가져오지 못하면 URL host를 기본 이름으로 사용합니다. 로그인 상태나 JavaScript 실행이 필요한 제목은 가져오지 못할 수 있습니다. 그룹·태그는 `links.json`에서 편집합니다. 저장 시 `http`·`https`·`mailto`만 허용하고 URL 구문을 검사합니다. 손상된 `links.json`은 덮어쓰지 않고 파일을 열어 고칠 수 있는 알림을 표시합니다.
...
- *열려 있는 파일 즐겨찾기에 추가*를 실행하면 **파일만 등록**(기본 선택) 또는 **현재 줄 포함 — 줄 N**을 고릅니다. 파일만 등록하면 `line`을 생략하며, 현재 줄을 포함하면 명령을 실행한 시점의 커서 줄을 저장합니다. 다른 곳을 클릭해도 선택 창은 유지됩니다. 같은 파일을 파일 단위와 특정 줄 단위로 각각 등록할 수 있고, 선택을 취소하면 저장하지 않습니다.
- 손상된 `favorites.json`은 덮어쓰지 않고 파일을 열어 고칠 수 있는 알림을 제공합니다.
- 항목을 클릭하면 파일을 엽니다. 저장된 줄 번호가 있으면 해당 줄로 이동하고, 없으면 특정 줄로 이동하지 않아 VS Code가 이전 편집 위치를 복원할 수 있습니다. 제목 표시줄에서는 검색·파일 편집을, 항목 우클릭 메뉴에서는 확인 후 삭제를 수행합니다.
...
TaskHub가 활성화되면 공개 GitHub [MunseopLim/TaskHub 릴리스](https://github.com/MunseopLim/TaskHub/releases)의
새 버전을 백그라운드에서 하루에 최대 한 번 확인합니다. `taskhub.updates.mode`로 확인·설치 방식을
정하며, 자세한 옵션은 [§21 설정 레퍼런스](#21-설정-레퍼런스)를 참고하세요.

- 기본 알림 모드에서는 새 버전의 **업데이트 설치**, **릴리스 보기**, **이 버전 건너뛰기**를 선택할
  수 있습니다. 알림을 닫으면 설치를 미룹니다. 건너뛴 버전은 알림 모드의 자동 확인에서 다시 알리지
  않으며, 그다음 새 버전은 다시 알려 줍니다. 자동 설치 모드는 건너뛴 버전도 설치합니다.
- 자동 설치 모드에서는 새 버전의 VSIX를 다운로드·검증한 뒤 설치합니다. 설치 후 재시작을 제안하며,
  사용자가 선택하기 전에는 창을 자동으로 다시 로드하지 않습니다. 설치 완료 알림을 열어 두어도
  다음 자동·수동 업데이트 확인은 계속할 수 있습니다.
- TaskHub 액션이 실행 중이면 다운로드를 시작하지 않으며, 다운로드 중 액션이 시작돼도 설치 전에
  멈춥니다. 자동 설치 모드는 확인한 릴리스를 기억해 1분 뒤 다시 시도합니다. 직접 설치를 선택한 경우에는
  액션을 마친 뒤 다시 확인하라는 안내를 표시합니다. 창 다시 로드도 실행 중인 액션이 없을 때만 수행합니다.
- Command Palette 또는 [기능 런처](#status-bar-기능-런처)의 **TaskHub: 업데이트 확인**
  (`taskhub.checkForUpdates`)으로 언제든 직접 확인할 수 있습니다. 자동 확인을 꺼도 이 명령은 사용할
  수 있으며, 하루 간격과 건너뛴 버전에 관계없이 확인하고 설치 여부를 묻습니다.

배포 대상은 위 공개 저장소로 고정되어 있으며, 사내 GitHub와 사용자 지정 저장소·인증은 지원하지
않습니다. 인터넷에 연결되지 않으면 자동 확인 실패는 TaskHub 출력 채널에 기록하고, 수동 확인에서는
실패 원인을 알립니다. 확인에 실패해도 다음 자동 확인까지 하루를 기다리며, 수동으로 다시 확인할 수
있습니다.
개발·테스트 실행에서는 자동 요청을 시작하지 않습니다. Restricted Mode의 동작은 문서 처음의
[사용 안내](#taskhub-상세-기능-문서)를 따릅니다.
...
C/C++ 구조체/클래스 선언에 마우스를 올리면 설정된 타입 크기와 정렬을 기준으로 **추정 크기**, 멤버별 오프셋, 패딩을 표시합니다. 실제 빌드의 `sizeof` 결과를 대신하지 않습니다.
...
*   **커스텀 타입 지원**: 문서 내에서 레이아웃을 해석할 수 있는 struct/class를 인식하여 중첩 타입 크기 계산
*   **C++ 데이터 멤버**: `int x{};`, `int x = 1;` 같은 초기화는 크기에 영향을 주지 않으며, static 멤버와 일반 비가상 메서드는 인스턴스 크기에서 제외
...
- 기본값은 `long`·포인터 4바이트, `long long`·`double` 8바이트, packing 상한 8바이트입니다. 대상 ABI에 맞게 [커스텀 타입 설정](#154-커스텀-타입-설정-taskhub_typesjson)을 조정하세요.
- 숫자 상수 차원의 배열, 해석 가능한 struct/union 멤버를 지원합니다. 비트 필드는 선언 타입의 저장 단위를 사용하는 추정이며, 컴파일러별 배치 규칙까지 재현하지 않습니다.
- `alignas`·레이아웃 속성, 상속, 가상 멤버, 해당 선언에 적용 중인 소스의 `#pragma pack`, 매크로 배열 차원이나 알 수 없는 멤버 타입이 있으면 크기를 확정하지 않고 계산 불가 안내를 표시합니다. 중첩 타입 해석에 실패한 경우도 부모의 정상 결과로 사용하지 않습니다.
- `#include`·조건부 전처리나 C++ 컴파일러의 전체 문법 분석을 수행하지 않습니다. 포함된 헤더에서 바뀐 packing도 재현하지 않습니다. 실제 크기·정렬·비트 필드 배치는 대상 컴파일러의 `sizeof`·`alignof`로 확인하세요.
...
현재 Listing 파일을 다시 분석합니다. 파일 변경은 자동으로 반영되지 않으며, 패널을 처음 열면
상태 영역에 수동 새로 고침이 필요하다는 안내를 표시합니다. 빌드 후에는 Refresh로 최신 결과를 확인하세요.
다시 읽는 범위는 Refresh의 마우스 도움말 또는 키보드로 버튼에 포커스했을 때 나타나는 설명에서
확인할 수 있습니다. 분석이 오래 걸리면 중복 Refresh를 허용하지 않고 기다려야 함을 상태 영역에 안내합니다.
...
GNU `ORIGIN`·`LENGTH`는 10진수·16진수·8진수 정수, `K`/`M` 접미사, 괄호 및 `+ - * / %` 상수 연산을 지원합니다. 예를 들어 `LENGTH = 128K - 4K`는 124KB, `LENGTH = (64K - 1K)`는 63KB입니다. 심볼·매크로·함수 호출·시프트 등 그 밖의 표현식은 일부 숫자만 읽지 않고 미지원으로 처리합니다.

Scatter는 `NAME 시작주소 [속성] 크기 { ... }` 헤더를 지원하며 줄바꿈을 허용합니다. 속성은 크기 앞에 둡니다.
Load 영역은 `ABSOLUTE`·`NOCOMPRESS`, execution 영역은 `ABSOLUTE`·`FIXED`·`NOCOMPRESS`·`UNINIT`을 지원합니다.
Load 영역의 크기는 생략할 수 있지만, execution 영역은 메모리 용량을 알 수 있도록 상수 크기가 필요합니다.
고정 주소와 속성을 사용하는 [Scatter 예제](../examples/sample_memory.sct)를 참고하세요.

첫 execution 영역의 `+offset`은 상위 load 시작주소를 기준으로 계산합니다. 후속 영역의 상대주소는 이전
영역의 실제 링크 크기가 필요하므로 추정하지 않습니다. `PI`·`RELOC`·`ALIGN`·`EMPTY` 등 그 밖의 속성,
심볼·산술식·전처리 지시문·닫히지 않은 블록도 불완전 입력으로 알립니다. 실제 주소를 확인하려면 ARM linker
listing을 사용하거나 `memoryMap.regions`에 전체 영역을 직접 정의하세요.

`ScatterAssert(...)`는 영역 선언과 구분합니다. 조건을 평가하지 않았다는 안내와 함께 정상 영역을 표시하며
**Refresh**도 허용합니다. 조건의 참·거짓은 실제 링커 빌드 결과에서 확인하세요.

선택한 링커/스캐터 파일에서 영역 자체를 해석하지 못하면 불완전한 목록을 사용하지 않습니다. 최초 열기는
저장된 영역 또는 ELF `PT_LOAD`로 폴백하고, **Refresh** 실패는 기존 결과를 유지합니다. GNU 심볼이나 함수가
포함된 영역도 동일합니다. 해석한 일부 영역만으로 전체 메모리 사용률을 계산하지 않습니다.
...
- Hex는 `48656C6C6F`, `48 65 6C 6C 6F`, `0x48, 0x65` 형식을 모두 지원합니다. 입력을 마치고
  ...
  입력 중에는 커서·선택 범위와 원문을 유지하므로 중간 바이트나 구분자를 그대로 수정할 수 있습니다.
- **한 줄 바이트 수**에서 8·16·32바이트 중 하나를 선택합니다. Hex 입력 왼쪽에는 첫 행
  `0x00000000`부터 각 행의 byte offset이 표시되며, Hex를 세로로 스크롤하면 함께 이동합니다.
  직접 줄을 나누면 앞선 행의 실제 바이트 수로 계산하며, 잘못되거나 불완전한 행 뒤에는 `—`로
  표시합니다. 포커스를 옮겨 행을 정리하면 오프셋도 함께 갱신됩니다.
  큰 입력의 변환을 기다리는 동안에는 첫 행만 `0x00000000`, 나머지는 `—`로 표시하며 계산 후 갱신됩니다.
- Hex 표시 단위는 공백 위치만 바꾸며 원본 바이트 순서는 바꾸지 않습니다. Little-Endian/Big-Endian
  ...
  표시 단위·행 크기·바이트 순서를 바꿔도 잘못된 UTF-8/ASCII 입력의 오류는 유지됩니다.
...
- 입력·한 줄 바이트 수는 같은 탭의 Webview가 다시 로드되어도 복원됩니다. 마지막으로 사용한
  **Text encoding**, **Hex grouping**, **Numeric byte order**는 패널을 닫거나 VS Code를 다시 열어도
  다음 변환기의 초기값으로 유지됩니다. 실시간 변환 입력은 1MB로 제한됩니다. 짧은 입력은 즉시
  변환하고, 큰 입력은 키 입력을 막지 않도록 120ms 동안
  이어진 변경을 한 번으로 합쳐 갱신합니다.

#### 비트 계산

**값 해석** 아래의 접을 수 있는 **비트 계산** 영역에서 레지스터·마스크 수식을 입력합니다.
입력 즉시 16진수·10진수·2진수 결과가 함께 표시되며, 각 결과를 별도로 복사할 수 있습니다.
비트 폭은 **8·16·32·64비트** 중에서 선택하고, 새 패널의 기본값은 32비트입니다.
64비트 정수도 정밀도 손실 없이 계산합니다.

- 숫자는 `0x` 접두사로 16진수, `0b` 접두사로 2진수를 입력합니다. 접두사가 없으면 10진수이며,
  선행 0이 있어도 `010`은 10으로 읽습니다. 8진수는 지원하지 않으며 이 규칙은 수식 입력 아래에도
  표시합니다. 음수와 선택한 비트 폭의 최대값을 넘는 숫자는 오류로 안내합니다.
- 지원 연산자는 `&`, `|`, `^`, `~`, `<<`, `>>`와 괄호입니다. 우선순위는 높은 순서로
  `~` → `<<`·`>>` → `&` → `^` → `|`이며, 같은 우선순위의 이항 연산은 왼쪽부터 계산합니다.
  예를 들어 `(0x1234 >> 8) & 0xFF`의 결과는 10진수 18입니다.
- 부호 없는 정수로 계산하고 **각 연산의 결과**를 선택한 비트 폭으로 제한합니다.
  8비트에서 `~0x0F`는 `0xF0`, `0x80 << 1`은 `0x00`입니다.
  따라서 `(0x80 << 1) >> 1`도 `0x00`입니다.
- `>>`는 왼쪽을 0으로 채우는 논리 이동입니다. 이동 횟수는 0부터 비트 폭−1까지만 허용합니다.
  64비트에서 `0x8000000000000000 >> 63`은 1이며, `1 << 64`는 오류입니다.
- 16진수와 2진수 결과는 선택한 비트 폭에 맞게 앞을 0으로 채우며, 복사할 때도 `0x`·`0b` 접두사를
  포함합니다. 잘못된 수식은 이전 결과와 복사 기능을 즉시 비웁니다. 입력을 이어가는 동안에는
  중립 상태를 표시하며, 350ms 멈추면 오류 위치를 안내합니다. 포커스를 옮기거나 비트 폭을 변경하면
  오류를 즉시 표시합니다.
...
주석 줄은 계수하지 않습니다. Intel HEX 주소 확장으로 식별 가능한 레코드가 손상되면 다음 정상 주소
확장까지의 데이터는 주소를 확정할 수 없어 별도로 제외하고 원본 재생성을 안내합니다. 접두사나 레코드
타입까지 깨지면 주소 확장 여부를 식별할 수 없으므로, 제외 안내가 있으면 표시 주소도 원본과 확인하세요.
...
Endian 변경은 선택한 숫자 해석과 Value 검색 결과에도 즉시 반영됩니다. Unit 변경은 기존 선택과 상태 표시를 초기화합니다.
...
| --- | --- | --- | --- | --- |
| `taskhub.updates.mode` | `"off"` \| `"notify"` \| `"auto"` | `"notify"` | 공개 GitHub 릴리스의 자동 확인 방식. `off`는 자동 확인 중지, `notify`는 새 버전 알림, `auto`는 검증 후 자동 설치. 재시작은 항상 사용자 선택이며 수동 확인은 모든 모드에서 가능하다. 머신 범위 설정으로 워크스페이스에서 덮어쓸 수 없다. | [§7 GitHub 릴리스 업데이트](#github-릴리스-업데이트) |
