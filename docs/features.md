...
- ELF에 DWARF 2~4 `.debug_line` 정보가 있으면 함수 행의 **소스 열기**에서 기록된
...
- stripped 바이너리, DWARF가 없는 파일, 아직 지원하지 않는 DWARF 5·DWARF64·압축
  line section은 소스 열을 숨기고 기존 섹션·심볼 분석은 그대로 제공합니다. 압축 플래그가
  붙은 `.debug_line`은 파서 오류 대신 아직 지원하지 않는 형식임을 안내합니다.
