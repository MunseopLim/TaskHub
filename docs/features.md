...
액션을 **실행하지 않고** 파이프라인이 어떻게 해석되는지 미리 보는 기능입니다. 메인 뷰에서 액션 우클릭 → **Preview Run (Dry-run)** 또는 Command Palette에서 `TaskHub: Preview Run (Dry-run)`.
...
*   **예제 JSON 보기**: 메인 패널 제목 표시줄의 전구(💡) 아이콘을 클릭하여 각 설정 파일의 예제 JSON 내용을 확인할 수 있습니다.
