...
- 액션 항목 클릭은 행을 선택할 뿐 실행하지 않습니다. 도구 항목은 클릭하면 저장된 파일과 열기 옵션으로 다시 엽니다. 액션은 행의 재실행 버튼이나 컨텍스트 메뉴처럼 의도가 분명한 명령으로만 다시 실행합니다.
...
- 행의 인라인 버튼으로 새 입력 재실행, 저장 입력 재실행, 명령 보기, 실패 출력 보기, 실행 보고서, 개별 삭제를 수행합니다. 제목 표시줄에서는 확인 후 전체 삭제합니다.
...
- Memory Map을 연 뒤 ELF 파일이 다시 빌드되거나 교체되면 오래된 오프셋을 사용하지 않고
  맵을 다시 열도록 안내합니다. ARM Linker Listing에는 대응 바이너리가 없으므로 이 열을
  표시하지 않습니다.
