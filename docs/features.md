...
- **확장 예산 (0.6.58부터)**: 깊이 제한만으로는 부족합니다 — `#define Mn M(n-1) M(n-1)` 형태는 깊이가 얕아도 결과가 2^n 으로 커져 hover 하나가 Extension Host 를 세울 수 있습니다. 결과 문자열 **64KB**, 치환 **20,000회**, 표시 단계 **500줄** 중 먼저 걸리는 쪽에서 멈추고, 확장 결과 대신 오류 문구를 표시합니다. 실제 헤더의 매크로는 이 한도에 닿지 않습니다.
- **공유 하위식은 한 번만 확장**합니다. 같은 매크로가 여러 번 나오면 처음 결과를 재사용하므로 그 하위 단계는 반복해서 표시되지 않고, 대신 마지막에 몇 번 접혔는지 한 줄로 요약됩니다. 순환 참조에 얽힌 매크로는 결과가 "누가 위에서 확장 중인가"에 달라지므로 재사용 대상에서 제외되며, 재사용이 깊이 제한을 우회하지도 않습니다(같은 정의 집합이면 토큰 순서와 무관하게 같은 답).
