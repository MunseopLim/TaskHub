...
- **파일 크기 한도**: 파일을 열거나 다시 읽을 때 10MB(10 × 1024 × 1024바이트) 한도를 적용합니다. 이미 연 파일은 편집 중 한도를 넘어도 계속 저장할 수 있습니다. 외부 변경으로 원본이 커진 뒤 **현재 편집 유지**를 선택한 경우에도 저장할 수 있으며, 원본 숫자의 손실 검사는 JSON Editor가 마지막으로 저장한 뒤 원문이 바뀐 경우에만 다시 수행합니다. 외부에서 바뀐 원문이 64MB를 넘으면 손실 여부를 확인할 수 없어 저장을 중단하며, 표의 편집 내용은 유지됩니다. 한도를 넘겨 저장한 파일을 닫았다가 다시 열려면 원문 편집기에서 크기를 줄여야 합니다.
