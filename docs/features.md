...
- **대비·타깃 크기 (0.7.0부터)**: 삭제 버튼은 배경으로 설계된 테마 토큰을 써 다크 테마에서 5.49:1 (WCAG 1.4.3 AA 통과). 작은 버튼과 변환 배지는 최소 24×24 클릭·터치 영역을 가집니다 (WCAG 2.2 SC 2.5.8 AA). 표가 답답하면 `--touch-min` 한 줄로 조절할 수 있습니다.
