...
- 목록은 TaskHub 사이드바·설정·업데이트 확인·[새로운 기능](#새로운-기능-whats-new), 액션 도구, 임베디드 도구, 미리 보기로 나뉩니다. 항목의 제목과
...
- **숫자 원문 보호**: 안전한 정수 범위를 벗어난 값, 무한대·0으로 넘치거나 정밀도를 잃는 숫자는 표 편집·저장을 거절하고 원문을 보존합니다. 원문 편집기를 사용하거나 큰 ID를 JSON 문자열로 표현하세요. 기존 숫자 문자열은 문자열로 유지합니다. 외부 변경·복구·객체/배열 셀에도 같은 검사를 적용합니다.
- **특수 키 보존**: `__proto__` 같은 키도 일반 JSON 속성으로 열고 편집·복구·저장합니다.
- **파일 크기 한도**: 파일을 열거나 다시 읽을 때 10MB(10 × 1024 × 1024바이트) 한도를 적용합니다. 이미 연 파일은 편집 중 한도를 넘어도 계속 저장할 수 있습니다. 외부 변경으로 원본이 커진 뒤 **현재 편집 유지**를 선택한 경우에도 저장할 수 있으며, 원본 숫자의 손실 검사는 JSON Editor가 마지막으로 저장한 뒤 원문이 바뀐 경우에만 다시 수행합니다. Windows에서는 파일 시간·크기가 같아도 원문을 일정 크기씩 읽어 내용이 같은지 확인하므로 큰 파일의 연속 저장에는 추가 읽기 시간이 듭니다. 외부에서 바뀐 원문이 64MB를 넘으면 손실 여부를 확인할 수 없어 저장을 중단하며, 표의 편집 내용은 유지됩니다. 직접 저장한 원문이 그대로이면 64MB를 넘어도 계속 저장할 수 있습니다. 한도를 넘겨 저장한 파일을 닫았다가 다시 열려면 원문 편집기에서 크기를 줄여야 합니다.
- **잘못된 셀 차단**: 객체·배열 셀의 JSON 파싱이 실패하면 편집 상태와 오류를 유지하고 저장하지 않습니다. JSON을 올바르게 고쳐 반영하면 이전 오류 안내를 지우고 저장할 수 있습니다.
- **Undo / Redo**: 셀 commit, 행 추가·삭제·정렬, 값 변환을 최대 20단계·16MB까지 되돌립니다. 셀 입력 중에는 브라우저 input의 undo가 우선합니다.
- **Dirty-close 복구**: commit된 변경은 워크스페이스 상태에 저장합니다. 다시 열 때 파일의 mtime·size가 같으면 복구를 묻고, 외부 변경이 확인된 오래된 복구본은 제안하지 않고 보관합니다. 복구 대상에 지원 불가 숫자가 있으면 안내와 **버리기** 선택을 제공하며, 알림을 닫으면 복구본을 유지합니다. 아직 파싱되지 않는 셀의 편집 중 raw text는 스냅샷에 포함하지 않습니다.
...
액션 우클릭 → **상단에 고정…**에서 일반 실행 또는 저장된 [입력 프로필](#14-액션-실행-히스토리)을
선택하면 Actions 목록 위쪽에 바로 실행할 수 있는 고정 항목이 생깁니다. 예를 들어 같은 빌드 액션을
`Board A · Debug`와 `Board B · Release` 프로필로 각각 고정해 입력 조합을 매번 고르지 않고 실행할 수 있습니다.

- 일반 실행을 고정하면 원래 액션과 같은 입력 절차를 거칩니다. 프로필을 고정하면 저장된 입력을 검증해
  재사용하며, 현재 선택지·검증 규칙과 맞지 않는 값은 다시 묻습니다. 태스크 ID·종류가 바뀐 오래된 프로필은
  기존 입력 프로필 실행과 같은 확인을 거칩니다.
- 고정 항목은 원본 액션이나 프로필을 복사하지 않습니다. 같은 ID의 액션 제목이나 프로필 이름을 바꾸면
  표시도 따라 바뀌며, 프로필의 값을 교체하면 다음 실행부터 새 값을 사용합니다.
- 액션이나 프로필이 삭제되면 고정 행에 없는 대상을 안내하고 실행하지 않습니다. 다른 입력으로 자동 실행하지
  않으며, 해당 행의 고정 해제로 정리할 수 있습니다. 고정 해제는 원본 액션과 프로필을 삭제하지 않습니다.
- 입력 프로필 저장소를 읽을 수 없으면 이유를 안내하고 **액션만 고정**을 제공합니다. 기존 프로필 데이터는
  그대로 보존합니다.
- 고정 목록은 이 VS Code 워크스페이스에만 저장하고 창을 다시 열어도 유지합니다. 팀의 `actions.json`이나
  프리셋에는 추가하지 않으며, 같은 액션·프로필 조합을 다시 고정해도 항목을 중복으로 만들지 않습니다.
...
Command Palette와 [기능 런처](#status-bar-기능-런처)의 **TaskHub: 새로운 기능** (`taskhub.showWhatsNew`),
Actions 제목 표시줄의 **`…` → 새로운 기능**에서 업데이트 내용을 확인합니다. 설치된 확장에 포함된
`CHANGELOG.md`의 버전별 제목과 본문을 사용하므로 인터넷 연결 없이 열 수 있습니다.

- 처음 이 기능을 사용한 버전은 기준 버전으로 기억합니다. 과거 변경을 모두 읽지 않은 항목으로 표시하지
  않으며, 명령을 직접 실행하면 현재 버전의 내용을 볼 수 있습니다.
- 이후 확인하지 않은 업데이트가 있으면 상태 표시줄의 TaskHub 옆에 점으로 알립니다. 미확인 버전 수는
  기능 런처와 상태 표시줄 도움말에서 확인합니다. 여러 버전을 건너뛰어 업데이트해도 해당 범위의 변경을
  모아 최신 버전부터 보여 주며, 시작할 때 알림이나 문서를 자동으로 열지 않아 원하는 때 확인할 수 있습니다.
- 목록에서 버전과 요약 제목을 보고 한 항목을 선택하면 Markdown 미리 보기로 상세를 엽니다. 미리 보기를
  사용할 수 없으면 읽기 전용 원문을 엽니다. 확인할 버전이 여러 개면 전체 업데이트 내용도 한 번에 열 수
  있습니다. 상세가 정상적으로 열린 항목만 읽음으로 처리하며, 선택을 취소하거나 열기에 실패하면 확인하지
  않은 상태를 유지합니다.
- 같은 VS Code 환경의 여러 창에서 읽은 기록은 함께 보존하며, 목록을 열 때 다른 창의 기록도 반영합니다.
  읽음 기록을 저장하지 못하면 안내하고, 다음 실행에서 다시 표시될 수 있습니다.
- 버튼과 안내는 VS Code 언어 설정을 따르며, 변경 내용은 한국어 원문을 표시합니다. 영어 UI에서는
  원문이 한국어임을 안내합니다. 전체 변경 이력은 기존 `…` 메뉴의 변경 이력 명령으로 열 수 있습니다.
...
- 기본 **재실행**은 입력을 새로 묻고, **저장 입력으로 재실행**만 `inputBox`·`quickPick`·`envPick`·`fileDialog`·`folderDialog`·`pathDialog`·`confirm`의 저장 결과를 사용합니다. 태스크 타입이 바뀌었거나 저장 당시 타입 정보가 없는 이전 기록, 현재 결과 구조·검증 규칙·선택지와 맞지 않는 값은 다시 묻고, `password: true`와 환경변수·클립보드·선택 텍스트에서 파생된 입력은 저장하지 않습니다.
...
Hex·Dec·Bin 값 옆의 복사 아이콘을 누르면 해당 숫자 문자열만 클립보드에 복사됩니다.
`0x`·`0b` 접두사와 앞쪽의 0을 포함해 화면에 표시된 형식을 유지하며, 항목 이름이나 설명은
포함하지 않습니다. 숫자 리터럴·식별자·매크로의 변환값, 레지스터 전체 값과 디코딩된 비트 필드,
[비트 연산 호버](#161-bit-operation-hover)의 Before/After 또는 Left/Result 각 값에서 사용할 수
있습니다. SFR 호버의 Bit Mask에도 복사 아이콘이 제공됩니다. VS Code의 기존 호버 전체 복사도
계속 사용할 수 있습니다.

음수 계산 결과는 `-0x5`·`-0b101`처럼 부호를 접두사 앞에 표시하며, 앞자리 0을 채운 값도 같은
규칙으로 복사됩니다. 진법 변환과 개별 복사는 정확한 정수에만 제공됩니다. 입력·중간 계산·결과가
소수·`Infinity`·`NaN`이거나 정수 정밀도를 보장할 수 없으면 복사 버튼을 제공하지 않습니다.
큰 레지스터 숫자 리터럴은 비트 필드 디코딩 대신 정확한 기본 숫자 변환을 표시합니다.

> **응답성 보호**: 한 Hover의 정의 탐색·문서 열기·후속 LSP 조회는 총 3초와 최대 16개 후보를 공유합니다. 취소하거나 예산을 넘으면 후속 탐색을 멈추고, 확인하지 못한 후보를 일치하는 정의로 간주하지 않습니다. 자동 workspace symbol 전체 검색은 수행하지 않습니다. 숫자 리터럴의 기본 변환은 가능한 경우 유지하며, 10,000자를 초과하는 줄에서는 Hover를 생략합니다.

숫자·SFR·레지스터로 해석되는 후보가 없으면 일반 함수·변수에 불확정 경고를 추가하지 않습니다. 숫자 값은 값을 얻은 후보끼리 비교하며, `extern const int X;`처럼 값이 없는 선언이나 그 선언의 조회 지연은 다른 정의에서 얻은 값을 가리지 않습니다. 서로 다른 숫자 값이 확인되면 불확정 안내를 표시합니다. 정의 조회나 후보 파일·Hover 조회가 시간 초과·실패하면 그 후보를 값이 없는 것으로 취급하지 않고 **확인하지 못함**으로 나열하며, 확인된 값은 표시하되 확인하지 못한 후보가 있다는 안내를 덧붙입니다. SFR·레지스터는 실제 후보가 있는 상황에서 정의가 충돌하거나 일부를 확인하지 못한 경우에 안내합니다. `size_t` 같은 일반 타입의 대입은 기본 숫자 변환을 유지하며, 타입을 사용하는 줄을 레지스터 정의로 세지 않습니다.
...
*   **다중 정의 지원**: 동일한 이름의 SFR이 여러 헤더 파일에 정의되어 있을 경우:
    *   모든 후보의 해석이 같으면 상세 테이블과 후보 경로를 함께 표시
    *   값·비트 필드·레이아웃이 다르거나 미확인 후보가 있으면 불확정 안내와 후보 경로를 표시
    *   파일 경로를 클릭하면 해당 위치로 이동하며, 현재 문서의 실제 비트 필드 선언은 그 선언을 기준으로 표시
...
`#define`으로 정의된 매크로 식별자 위에 hover 하면 최종 계산 결과가 표시됩니다. 다른 매크로를 참조하는 compound/nested 매크로도 재귀 확장하며, 정수식(`+ - * / | & ^ << >> ()`)의 결과를 계산합니다. 값의 표시·복사 조건은 [C/C++ Hover의 값 복사](#15-cc-hover-기능)를 따릅니다.
...
- 확장된 표현식은 공백을 포함해 4096자 이하일 때 평가합니다. 초과 시 숫자 변환을 제공하지 않습니다.
...
마지막으로 사용한 **Unit**, **Endian**, **찾기 방식**은 Hex/Text 변환기처럼 패널을 닫거나
VS Code를 다시 실행해도 유지됩니다. 새 파일이나 Custom Editor로 여는 뷰어의 초기값에 적용되며,
이미 열려 있는 탭은 각자 선택한 설정을 웹뷰 재로드 후에도 유지합니다.
...
`Ctrl/Cmd+F` 또는 **Find**로 찾기 바를 열고 **찾기 방식**을 먼저 선택합니다. 저장된 설정이 없을 때는 **값 (Value)**로 시작합니다.
...
- 좁은 창에서 선택 정보가 여러 줄로 표시되어도 키보드·주소 이동·검색으로 선택한 셀은 표시 영역 안으로 스크롤
...
긴 패턴 검색도 작업을 나누어 화면 입력에 응답하며, 검색어를 바꾸거나 검색 창을 닫으면 진행 중인 검색을 취소합니다. 검색 결과 강조는 보이는 셀 범위에만 적용합니다.
