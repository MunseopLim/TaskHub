...
- **키워드 검색**: 상단 검색창에서 오브젝트/섹션/함수 이름, 주소, 크기, 타입으로 필터링. 접힌 region 내부도 검색되며 매치 시 자동으로 펼침. `Ctrl/Cmd+F`로 검색창에 포커스(기존 입력은 전체 선택), `Esc`로 검색어 초기화 후 다시 한 번 누르면 포커스 해제
  - **매치 하이라이트**: Region Details 테이블(가상 스크롤 포함)과 All Sections / Overview 테이블 모두에서 검색어와 일치하는 부분 문자열을 에디터 "찾기" 강조색으로 칠해 위치를 즉시 알 수 있습니다
  - **검색창 상단 고정**: 검색 박스가 화면 상단에 고정되어, 결과를 보러 아래로 스크롤해도 입력창과 매치 수가 계속 보입니다
  - **첫 매치로 자동 스크롤**: 검색 직후 화면 밖에 있는 첫 매치를 자동으로 가운데에 가져옵니다(이미 보이면 스크롤하지 않음)
  - **매치 수 표시**: 검색창 오른쪽에 `27 matches in 4 regions` 형태로 매치 개수와 매치가 있는 region 수를 표시하고, 결과가 없으면 `No matches`를 경고색으로 표시
