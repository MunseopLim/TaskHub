...
### 3a. 소스 → 맵 점프  *(0.7.14에 구현됨)*

상단 "이미 구현된 항목" 표 참조. 실측대로 얇게 끝났고(신규 코드 대부분이 매칭 판정), 착수 시 예상하지 못했던 것은 하나였습니다 — **C++ mangled 이름**. 소스에는 `HAL_Init` 로 쓰지만 맵에는 `_ZN3HAL8HAL_InitEv` 로 들어 있어, 이름 비교만으로는 C++ 프로젝트에서 거의 아무것도 찾지 못합니다. 디맹글러 없이 Itanium ABI 의 `길이+이름` 표기를 근거로 판정해 해결했습니다(§3c 의 디맹글링 부채와는 별개 — 이쪽은 *찾기*만 하면 되고 *복원*할 필요가 없어서 가능했습니다).
