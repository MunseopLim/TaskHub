...
| P0 | Memory Map 빠른 열기·Refresh | 소 | ELF 분석을 반복할 때 대화상자와 재입력을 줄임 |
...
## P0 — Memory Map 빠른 열기·Refresh

- Explorer의 `.elf`, `.axf`, `.out` 컨텍스트 메뉴에서 Memory Map으로 바로 열 수 있게 합니다.
- 열린 패널에서 현재 ELF와 기존 linker/scatter 선택을 다시 입력하지 않고 수동 Refresh할 수 있게
  합니다.
- Refresh가 실패하면 이전 결과를 보존하되 최신 결과로 오인하지 않도록 실패 상태를 표시합니다.

자동 파일 감시와 빌드 중 갱신은 포함하지 않습니다. 현재 구현된 ELF `PT_LOAD` 기반 영역 감지도
새 기능으로 다시 추적하지 않습니다.
