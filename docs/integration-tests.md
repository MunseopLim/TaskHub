...
두 개의 규칙 중 하나는 매칭되고(`hit`), 하나는 `line: 5`로 실패합니다. 실패한 규칙은 조용히 skip되고 파이프라인은 성공합니다. **실패한 파생 변수를 downstream에서 실제로 참조해** 그것이 `${src.third}` 리터럴로 남는 것까지 함께 고정합니다 — 예전에는 아무도 쓰지 않아 폴백으로 producer의 stdout 전체가 들어가도 테스트에 드러나지 않았습니다.

### IT-005b: capture miss가 stdout으로 대체되지 않음

IT-005와 같은 상황을 **셸 인자 자리**에서 확인합니다. 정규식으로 좁힌 값을 받는다고 믿는 자리에 검증되지 않은 출력 전체가 들어가면 안 되므로, `${src.safe}`가 리터럴 그대로 자식 프로세스에 전달되는 것을 검증합니다.
...
악성 아카이브가 `../outside.txt` 같은 엔트리 이름으로 대상 디렉터리 밖에 파일을 쓰려고 시도할 때, `extractZipArchive`는 추출 전에 모든 엔트리의 해석된 경로가 대상 디렉터리 안에 있는지 검증하고 벗어나면 "Blocked path traversal" 에러를 던집니다.

> **0.6.45부터 방어가 두 겹입니다.** 추출이 `yauzl` 스트리밍으로 바뀌면서, 그 라이브러리도 `../`·절대 경로 엔트리를 자체 규칙으로 거부합니다(`invalid relative path` / `absolute path`). 중복이지만 없애지 않았습니다 — 우리 검사가 훗날 실수로 약해져도 라이브러리 쪽에서 걸립니다. 다만 호출부와 테스트가 이 모듈의 메시지에 의존하므로, `normalizeZipError` 가 라이브러리 문구를 `Blocked path traversal` / `resolves to destination root` 로 번역합니다. **두 문구를 구분해 유지하는 이유**는 원인이 다르기 때문입니다 — 전자는 대상 밖으로 나가려는 시도, 후자는 대상 디렉터리 자체로 해석되는 엔트리(`a/..`)입니다. 테스트는 (1) 파이프라인이 실패하는지, (2) 대상 밖 경로에 실제 파일이 생성되지 않는지 둘 다 확인합니다. adm-zip이 `addFile` 시점에 `../`를 자동 정리하므로, 테스트는 `entryName`을 사후 수정하여 실제 공격자 기법을 모사합니다.
...
### IT-119 ~ IT-122, IT-147 ~ IT-149: 마법사 생성 종단 흐름

0.6.18의 저장 전 확인 단계와 0.6.25의 *ID 변경* 버튼은 마법사가 만든 액션 객체(`newAction.id`)를 제자리에서 고칩니다. 그런데 저장 이후 코드는 최초에 도출한 `id` 지역 변수를 계속 들고 있어서, ID를 바꾸고 *바로 실행*을 누르면 존재하지 않는 ID로 `executeActionById`가 불렸습니다 — 방금 만든 액션에 대해 "액션을 찾을 수 없습니다"가 뜨는 상태입니다.

0.6.17~0.6.26에 추가된 마법사 테스트 20여 종이 이걸 잡지 못한 이유는 전부 순수 함수 경계(`buildTasks`, `validateActionIdInput`, `buildWizardReviewDetail`)에서 멈췄기 때문입니다. 결함은 그 경계 **바깥**, 저장과 후속 실행 사이의 배선에 있었습니다. 그래서 이 그룹만 `taskhub.createAction` 명령 자체를 끝까지 돌립니다.

- **IT-119**: 확인 단계에서 ID를 `renamed-by-user`로 바꾼 뒤 저장하고 *바로 실행*까지 갑니다. 디스크의 `id`와 `executeActionById`에 넘어간 `id`가 **둘 다** 새 값이어야 합니다. 수정을 되돌린 상태에서 실제로 실패하는 것을 확인했습니다(IT-120은 그대로 통과 — 흐름이 아니라 결함을 겨냥한다는 증거).
- **IT-120**: ID를 바꾸지 않는 대조군. 제목에서 도출한 `build-firmware`가 저장·실행 양쪽에 쓰입니다.
- **IT-121**: 확인 단계를 취소하면 `.vscode/actions.json`을 아예 만들지 않고 실행도 하지 않습니다.
- **IT-122**: *자세히 보기* 이후의 재확인이 **비modal**임을 고정합니다. VS Code modal은 워크벤치를 가리고 입력을 잡아, 검토 문서를 연 직후 modal을 다시 띄우면 그 문서를 스크롤할 수 없습니다(0.6.18~0.6.26의 동작).
- **IT-147**: modal의 *Cancel* / Escape는 **명시적인 취소**이므로 되묻지 않고 한 번에 끝납니다. 실수로 닫히는 알림과 같이 취급하면 일부러 취소한 사용자가 두 번 닫아야 합니다.
- **IT-148**: 비modal 알림이 닫히면 초안을 살린 채 한 번 되묻고, *다시 검토*로 재진입한 확인 화면이 **여전히 비modal**임을 고정합니다. 검토 문서를 연 사실이 함수 지역 변수였을 때 재진입이 그것을 리셋해 IT-122가 막으려던 상태(열린 문서 위의 modal)가 되살아났습니다. 액션이 중복 삽입되지 않는 것도 함께 봅니다.
- **IT-149**: 되묻는 알림의 *버리기*를 고르면 저장하지 않고 끝납니다 — 초안을 버리는 선택이 이름 없는 X 제스처뿐이면 파괴적인 쪽이 가장 눈에 안 띄는 자리에 놓입니다.

프롬프트 응답은 **버튼 문구가 아니라 위치**로 지정합니다. 생산 코드는 `t('저장','Save')` 결과와 반환값을 비교하므로 대본에 한국어 문구를 박으면 한국어 호스트에서만 통과하고 영어 CI에서는 조용히 "취소"로 해석됩니다 — 로케일에 따라 결과가 달라지는 테스트는 없느니만 못합니다. 대신 버튼 **개수**를 함께 단언해 구성이 바뀌면 먼저 깨지게 했습니다.

### IT-123 ~ IT-125: 대화형 태스크 대기 중의 중지

`activeTasks`와 `actionChildProcesses`는 프로세스가 있는 작업만 담습니다. `inputBox` / `quickPick` / `fileDialog` 프롬프트 앞에 멈춰 있는 액션은 둘 다 비어 있으면서 상태는 `running`이라, 트리에 스피너와 중지 버튼이 떠 있는데도 `stopRunningAction`이 멈출 것을 찾지 못해 *"활성 태스크를 찾을 수 없습니다"* 경고만 띄웠습니다 — 중지하지 않는 중지 버튼이었습니다.

- **IT-123**: `showInputBox`를 "토큰이 취소되면 `undefined`로 resolve"하는 가짜(VS Code 실제 계약과 동일)로 바꾸고 실행 중 `stopRunningAction`을 호출합니다. 중지가 `true`를 반환하고, 프롬프트가 닫히며, 뒤 단계가 실행되지 않고, 히스토리가 `Action stopped by user`로 마감되는지 확인합니다. 토큰 배선을 되돌린 상태에서 실제로 실패하는 것을 확인했습니다.
- **IT-124**: 대조군 — 중지를 누르지 않았으면 토큰이 취소 상태가 아니어야 합니다. 이게 깨지면 정상 실행이 첫 검사에서 곧바로 중단됩니다.
- **IT-125**: 실행이 끝나면 취소 스코프가 정리되는지 확인합니다. 남아 있으면 **같은 액션의 다음 실행**이 이전 실행의 취소된 토큰을 보고 즉시 중단됩니다.

두 가지 제약이 이 그룹의 형태를 결정했습니다.
