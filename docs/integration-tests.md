...
| IT-186 | 내장 민감값 첫 반복 마스킹 | 환경변수에서 직접 파생된 `${each}`도 첫 반복부터 실제 실행에만 원문을 전달하고 History 명령·Action Run Report에서는 `***`로 가림 |
| IT-187 | 민감 파생 반복 경로 마스킹 | 첫 반복에서 `${each}`가 파일 경로에 들어가도 저장 알림에는 원문 경로 조각을 노출하지 않고 `***`로 표시 |
| IT-188 | 비민감 반복값의 식별성 보존 | 리터럴 `forEach`가 도는 task의 다른 필드에서 환경변수를 써도 `${each}`는 History 명령·Run Report에 남기고, 실제 민감 출력만 계속 가림 |
...
| IT-224 | [hexViewerBrowser.test.ts](../src/test/hexViewerBrowser.test.ts) | 464×388px의 좁은 창·14px 글꼴·15px 가로 스크롤바를 실제 브라우저에 재현하고, End·검색으로 이동한 마지막 셀 전체가 스크롤바를 제외한 콘텐츠 영역에 표시되는지 검증 |
| IT-225 | [jsonEditorBrowser.test.ts](../src/test/jsonEditorBrowser.test.ts) | 실제 브라우저에서 지원 불가 숫자 셀 입력이 저장을 막고 원본을 보존하며, 올바른 값으로 고친 뒤 저장되는지 검증 |
| IT-226 | [jsonEditorBrowser.test.ts](../src/test/jsonEditorBrowser.test.ts) | `__proto__`가 포함된 복구 데이터와 디스크 baseline을 실제 웹뷰로 전달하고 편집·저장 시 속성과 dirty 상태를 보존하는지 검증 |
| IT-227 | [hexViewerBrowser.test.ts](../src/test/hexViewerBrowser.test.ts) | 긴 바이트 패턴 검색을 새 입력으로 취소하고, 최신 검색이 파일의 마지막 바이트만 선택하는지 실제 브라우저에서 검증 |
| IT-228 | [jsonEditorBrowser.test.ts](../src/test/jsonEditorBrowser.test.ts) | 실제 웹뷰에서 열린 파일을 10MB 이상으로 늘린 뒤 다시 늘리거나 줄이며 연속 저장하고, 파일 내용·셀 값·dirty 상태가 일치하는지 검증 |
...
| `buildElf32WithDwarf5Lines()` | 외부 문자열 table을 참조하는 DWARF 5 함수별 소스 열기 |
