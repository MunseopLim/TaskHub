...
4.  **쉘 인자 이스케이프 / 실행 경로 선택**
    ...
    *   `password: true` 파생값을 **파일로 남기는 것**은 태스크의 `allowSecretContent` 선언이 있을 때만 수행한다. `editor`·`terminal`은 사용자가 위치를 고르지 않은 암묵적 영속화라 계속 전면 차단이고, `writeFile`·`appendFile`·`output.mode: 'file'`은 사용자가 적은 경로이므로 선언을 요구하는 쪽을 택한다 — 기능을 없애면 같은 일이 `shell`의 리다이렉션으로 내려가 마스킹·권한·기록이 모두 사라지기 때문이다. 선언된 쓰기는 `0600`으로 만들고 저장 사실을 알린다. 사용자 관점 설명은 [features.md의 password 절](./features.md#password-값이-가려지는-범위) 참조.
