...
│   ├── hexViewer.ts                   # Hex Viewer WebView (초기 file-offset 선택 포함)
...
3.  **파일 경로 검증(`resolveWithinWorkspace`)**
    *   Task output mode가 `file`일 때, 그리고 `writeFile` / `appendFile`의 `path`와 즐겨찾기 항목 경로에 대해, 치환 경로와 실제 경로를 워크스페이스 루트와 대조한다. 존재하는 조상은 `lstat`로 찾으므로 끊어진 심볼릭 링크도 검사하며, 실제 경로를 확인할 수 없으면 거부한다.
    *   세 파일 쓰기 경로는 `writeWorkspaceFileSync()`를 공유한다. 정상적인 내부 링크는 확인한 실제 대상으로 쓰고, 일반 파일이 아닌 대상과 hardlink를 거부한다. 최종 파일을 truncate 없이 열고 POSIX `O_NOFOLLOW` 및 열린 fd의 파일 종류·링크 수·device/inode 재검증 뒤에 내용을 쓴다. `overwrite: false`는 exclusive open으로 검사한다. 부모 디렉터리가 동시에 교체되는 모든 OS 경합을 원자적으로 격리하는 샌드박스는 아니다.
    ...
    *   **상대 경로 기준점**: `resolveTaskWorkingDirectory()`가 공통 cwd를 해석하고, `resolveArchiveTaskPath(targetPath, baseDir)`가 이를 기준으로 아카이브 대상 경로를 만든다. 내장·외부 엔진과 Preview가 이 헬퍼를 공유하며 `${zip.archivePath}` / `${unzip.outputDir}`는 해석된 절대 경로를 반환한다. 경로 격리는 적용하지 않으며, 사용자 설정 규칙은 [액션 작성 가이드](actions.md#command)를 따른다.
...
5.  **WebView 보안**
    ...
    *   JSON 저장 전 숫자 검사는 마지막 직접 저장의 파일 식별자·크기·고해상도 수정/변경 시각이 같을 때 생략한다. 캐시는 쓴 fd의 상태와 현재 경로의 상태를 비교해 등록하며, 외부 변경을 반영하는 saved baseline과 분리한다. 메타데이터 비교는 동시 쓰기를 잠그거나 원자적 저장을 보장하지 않는다. 검사 크기 한도와 실패 시 동작은 [JSON Editor 데이터 보호](features.md#데이터-보호)를 참조한다. 정상 디스크 파일의 복구본은 먼저 신선도를 판정하고 복구 대상에만 숫자 검사를 적용하며, 지원 불가 데이터를 사용자 선택 없이 삭제하지 않는다.
...
7.  **Hover 텍스트·복사 입력과 비동기 처리**
    ...
    *   `withLspTimeout()`은 지연 호출과 취소 토큰을 받으며, 요청별 공유 deadline이 LSP와 문서 열기에 남은 시간을 전달한다. 활성 호버와 진행 중인 LSP 호버의 위치를 추적해 동일 위치의 재귀 조회를 막고, 조회 종료·취소·시간 초과 시 추적 상태를 해제한다. 로컬 매크로를 표시할 때는 불필요한 숫자 조회를 시작하지 않는다. 정의·선언 후보 합의와 응답성 한도의 사용자 동작은 [C/C++ Hover](features.md#15-cc-hover-기능)를 참조한다.
...
8.  **링크 제목 조회**
    ...
    *   최초 요청과 매 리다이렉트마다 IP 리터럴 또는 모든 DNS 응답을 검사한다. loopback·사설·link-local·예약 주소와 IPv4-mapped/전환용 IPv6 주소를 거부하고, 실제 연결의 lookup에는 검증한 응답을 고정해 DNS 재조회에 의한 주소 변경을 막는다. DNS 대기 중 취소·기한 초과가 발생하면 뒤늦게 응답해도 연결하지 않는다.
