...
1. **히스토리 기능 수정 시**:
   ...
   *   [src/extension.ts](../src/extension.ts) `executeAction()` 의 히스토리 추적 호출 순서(`addHistoryEntry` → `updateHistoryStatus` → `setHistoryInputs`) 유지. 입력값 캡처는 `executeActionPipeline`의 `recordInputs` 옵션을 통해 누적되며, `password: true` `inputBox`는 `shouldRecordTaskInput`에서 명시적으로 제외된다 (회귀 시 `IT-045`로 검출됨).
