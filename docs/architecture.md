...
4.  **쉘 인자 이스케이프 / 실행 경로 선택**
    ...
    *   Windows — **native 경로**(계획 결과가 `native`일 때): `executeShellCommand`의 `spawn(file, argvArray)`, VS Code Task 경로의 `vscode.ProcessExecution`, one-shot의 `ProcessStartInfo`(`UseShellExecute=$false`) 모두 계획에 보존한 같은 절대 경로를 사용한다. 인자를 argv 배열 또는 `quoteWindowsCommandLineArgument`로 escape한 문자열로 직접 넘겨 **Windows PowerShell 5.1의 native-command 인자 큐오팅 버그**(`"` 가 사라지는 문제)를 우회한다. 판정 뒤 파일이 사라지거나 실행이 거부되면 원래 bare 이름을 PowerShell로 재해석하지 않고 시작 실패를 반환한다.
    *   Windows — **PowerShell 경로**(계획 결과가 `powershell`/`raw-shell`일 때): `buildPowerShellInvocation`이 `quotePowerShellArgument`로 각 인자를 싱글쿼트로 감싸 PowerShell `-EncodedCommand`로 전달한다. one-shot은 `Start-Process -FilePath … -ArgumentList @(…)`를 사용해 PATHEXT/파일 연결을 셸처럼 해석한다.
5.  **WebView 보안**
