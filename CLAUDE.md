...
**GitHub CI가 통과하도록 검증한 뒤 커밋한다.** 커밋 전 [CI 워크플로](.github/workflows/ci.yml)와 변경 내용에 관련된 워크플로의 검사 항목·실행 환경(OS, Node.js 버전 등)을 반드시 확인한다. 로컬에서 재현 가능한 CI 검사를 모두 통과한 뒤에만 커밋하며, 실패하면 원인을 수정하고 다시 검증한다.

현재 환경에서 실행할 수 없는 검사는 해당 항목과 사유를 명시한다. 로컬 검증 통과와 GitHub의 실제 CI 실행 결과를 구분하여 보고한다. 커밋·푸시 실행 권한은 위 "Git 커밋과 푸시 권한" 규칙을 따른다.
