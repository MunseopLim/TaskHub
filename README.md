...
*   **워크스페이스 링크 (`mainView.linkWorkspace`)**: 워크스페이스의 `.vscode/links.json`에 정의된 링크를 표시합니다. 제목에는 링크의 총 개수가 표시되며, 복사/열기/삭제가 가능합니다. 뷰 상단의 + 버튼과 연필 버튼을 통해 링크 추가/편집을 할 수 있습니다.

두 패널 모두 링크 항목 앞에 표준 링크 아이콘이 붙고, 클릭 시 기본 브라우저에서 URL이 열립니다.
...
이 패널은 `.vscode/favorites.json`에 정의된 사용자가 즐겨찾는 파일 목록을 표시합니다. 필요하다면 파일을 열 때 이동할 줄 번호까지 함께 저장할 수 있으며, 뷰의 제목은 "Favorite Files"로 즐겨찾기된 항목의 총 개수가 표시됩니다.

*   **즐겨찾기 추가**: 뷰의 제목 표시줄에 있는 "+" 아이콘을 클릭하여 기존 파일을 즐겨찾기에 추가할 수 있습니다. 선택한 각 파일에 대해 제목과 이동할 줄 번호(비워두면 파일 시작)를 입력합니다.
*   **열려 있는 파일 즐겨찾기에 추가**: 열려 있는 편집기에서 마우스 오른쪽 버튼을 클릭하여 컨텍스트 메뉴에서 "열려 있는 파일 즐겨찾기에 추가" 옵션을 선택하여 현재 파일을 즐겨찾기에 추가할 수 있습니다. 기본값으로 현재 커서 위치의 줄 번호가 제안되며 필요에 따라 수정하거나 비울 수 있습니다.
*   **클릭하여 열기**: 즐겨찾기 항목을 클릭하면 해당 파일이 VS Code에서 열립니다. 줄 정보가 있으면 해당 줄로 포커스를 이동합니다.
