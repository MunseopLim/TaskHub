...
릴리스 이력은 [CHANGELOG.md](CHANGELOG.md) 상단 작성 규칙에 따라 기록합니다. 업데이트 안내도 이 기록을 사용하며 별도의 릴리스 요약을 중복 작성하지 않습니다.
...
기능 추가·변경·삭제 및 버그 수정 시 [CONTRIBUTING.md — 기능 변경 검증 의무](CONTRIBUTING.md#기능-변경-검증-의무)를 반드시 따릅니다.
...
- **실행 코드·사용자 기능/설정/문구·운영 의존성·배포 내용이나 빌드 동작을 바꾸면 새 버전으로 올립니다.** 버그 수정, 플랫폼별 수정, CI 실패를 해결하기 위한 제품 코드 수정도 포함합니다.
- 이미 커밋된 버전에 새 동작 변경을 더하면 다음 버전으로 올립니다. 일반 수정은 patch 증가를 기본으로 합니다(예: `0.8.31` → `0.8.32`). 같은 미커밋 작업을 다듬는 동안에는 매번 추가로 올리지 않습니다.
- `package.json`, `package-lock.json`의 최상위와 루트 패키지 버전, `CHANGELOG.md`의 새 버전 항목을 함께 갱신합니다. 이미 커밋된 릴리스 항목에 새 수정 내용을 끼워 넣어 버전 증가를 대신하지 않습니다.
- 실행·배포 동작에 영향이 없는 테스트·문서(문서용 이미지 포함)·에이전트 지침·CI·개발 도구만 바뀌면 버전을 유지합니다.
- **커밋 허가와 관계없이 완료 보고나 커밋 승인 요청 전에 버전 갱신까지 마칩니다.** 아직 커밋·릴리스하지 않았다는 이유로 미루지 않습니다.
- 자동 검사와 비교 기준은 [CONTRIBUTING.md — 버전 검사](CONTRIBUTING.md#버전-검사)를 따릅니다. 검사 실패를 피하려고 기준을 바꾸거나 검사를 건너뛰지 않습니다.
...
- 버전 변경 여부와 동반 갱신은 위 [버전 관리](#버전-관리)를 따른다.
...
전체 워크플로 목록·실행 조건·환경·검사 명령은 [CONTRIBUTING.md — CI 워크플로와 커밋 전 검증](CONTRIBUTING.md#ci-워크플로와-커밋-전-검증)에서 관리한다. 워크플로를 추가하거나 변경하면 해당 문서도 함께 갱신한다.

**매 커밋 전, 현재 변경사항이 모든 GitHub Actions 워크플로에서 통과할 수 있는지 검토한다.** 위 문서와 [실제 워크플로 파일](.github/workflows/)을 대조하고, 태그·일정에서만 실행되는 워크플로를 포함해 검사 항목과 OS·Node.js 환경을 확인한다. 알려진 CI 실패가 있으면 해당 커밋과 job 로그를 확인하여 원인을 검토하고, 현재 변경에도 남아 있는 실패는 수정한다.

[커밋 전 체크리스트](CONTRIBUTING.md#커밋-전-체크리스트)에 따라 OS 차이와 테스트 종료·정리까지 검토하고, 로컬에서 재현 가능한 필수 검사와 변경 영향이 있는 CI 검사를 모두 통과한 뒤에만 커밋한다. 실패하면 원인을 수정하고 다시 검증한다.

현재 환경에서 실행할 수 없는 검사는 해당 항목과 사유를 명시한다. 로컬 검증 통과와 해당 커밋의 실제 GitHub 실행 결과를 구분하며, 로컬 통과만으로 Windows 등 미실행 환경의 CI 통과를 주장하지 않는다. 커밋·푸시 실행 권한은 위 "Git 커밋과 푸시 권한" 규칙을 따른다.
